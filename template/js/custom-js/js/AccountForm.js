      ...
      this.$nextTick(() => {
        ...
        if (!countInvalidInputs('.is-invalid')) {
          ...
          if (checkFormValidity($form)) {
            this.saveToStorage()
            this.save()
            if (window.ecomCart && window.ecomCart.data && window.ecomCart.data.items) {
              const dataFetch = {
                items: window.ecomCart.data.items,
                name: this.localCustomer.display_name,
                email: this.localCustomer.main_email,
                phone: this.localCustomer.phones
              }
              window.fetch('https://zeprint.bubbleapps.io/api/1.1/wf/ecomplus', {
                method: "POST",
